...
function make_recursion_make_menu(array $sections, $level = 0) 
{
    // $current_level = $level;
    foreach ($sections as $c) {
        ...
        if (!empty($c['children']))
            recursion_make_menu($c['children'], $level + 1);
...
function make_menu($array, $recursion = FALSE) 
{
    foreach ($array as $a) {
        echo '<li>
...
function make_breadcrumb($breadcrumb)
{
...
function make_file_table($array, $base_addr) 
{
